...
public class StaffFrame extends JFrame implements ActionListener, Runnable, CafeNet {
	...
	private Board board; // 홈
	private StaffStudy staffStudy;
	private StaffOrder staffOrder; // 주문현황
	private ChatRoom chatRoom; // 문의받기
	private Sales sales; // 매출현황
	private ManageMember manageMember; // 회원관리
	...
	public StaffFrame(MemberDTO member) {
		this.member = member;
		...
		String btnMenusName[] = { "홈", "스터디 예약현황", "주문 현황", "매출 현황", "회원 문의", "회원 관리" };
	...
	public void addCardPanels() {
		...
		staffStudy = new StaffStudy(member, this);
		staffOrder = new StaffOrder(member, this);
		sales = new Sales(member, this);
		chatRoom = new ChatRoom(member);
		manageMember = new ManageMember(this);

		pnlMenuWrap.setBounds(25, 250, 1210, 500);
		pnlMenuWrap.add(board, "board");
		pnlMenuWrap.add(staffStudy, "staffStudy");
		pnlMenuWrap.add(staffOrder, "staffOrder");
		pnlMenuWrap.add(sales, "sales");
		pnlMenuWrap.add(chatRoom, "chatRoom");
		pnlMenuWrap.add(manageMember, "manageMember");
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if (e.getSource() == btnMenuArr[1]) { // 스터디룸 현황
			btnColorChange(1);
			card.show(pnlMenuWrap, "staffStudy");
		} else if (e.getSource() == btnMenuArr[2]) { // 주문 현황
			btnColorChange(2);
			card.show(pnlMenuWrap, "staffOrder");
		} else if (e.getSource() == btnMenuArr[3]) { // 매출 현황
			btnColorChange(3);
			card.show(pnlMenuWrap, "sales");
		} else if (e.getSource() == btnMenuArr[4]) { // 회원 문의
			btnColorChange(4);
			card.show(pnlMenuWrap, "chatRoom");
			chatRoom.callChatRoom(member);
		} else if (e.getSource() == btnMenuArr[5]) { // 회원 관리
			btnColorChange(5);
			card.show(pnlMenuWrap, "manageMember");
